...
- [Expert Streaming and Chunked Prefill](../execution/active/EXPERT_STREAMING_EXECUTION_PLAN.md): the routed-expert supply — telemetry, the tier-invariance and starved-pool gates, the demotion-queue A/B, then chunked prefill with a chunk-wide expert sweep and dedup. Carries item 21's concurrency remainder and item 19's throughput half. Its working document supersedes [the analysis](../analysis/current/EXPERT_STREAMING_AND_CHUNKED_PREFILL_ANALYSIS.md), whose §6 (the prefill supply chain) stays deliberately open.
