...
**Nothing here is comparable to §4.** Those entries measured a different computation, on different
contexts, prompts and instrumentation; the silicon and the artifact are all the two sections share.
In every entry below decode is `≈3 tok/s` — unbatched decode with a one-token prefill — so none of
these numbers is a throughput result.

### M28: The rebuilt graph produces coherent text at `≈3 tok/s`
- **Run**: `2026-09-17`; branch `rewrite/graph-v2`; DeepSeek-V4-Flash-0731-INT4-W4A16-Aeon, 43 layers, 11,008 experts
- **Class / comparison key**: `E2E / e2e-rewrite-43L-text` — a new key; do not pool with `native-text` (M21–M26) or `e2e-43L-A/B`
- **Platform**: `baseline`, Device 0 only; RX 7900 XTX, ROCm 7.2.2, `gfx1100`, Wave32
- [ ] **Invalidate for comparison** | **Reason**: `--`
- **Workload / configuration**: acceptance prompt `What is the capital of France?` (`11` prompt tokens) and `8` single-turn thinking-mode prompts (riddles, an open explanation, a history list, a factorial derivation); artifact sampling policy (`T=1.0`, `top_p=1.0`); `n=1` per prompt
- **Metrics**:
  ...
  Prefill `302.89–340.26 ms/token` against decode step `274.73–292.40 ms` — `≈1.13×` decode per prompt token. VRAM held `23.75 GiB = 99.06%` at context `32768` (`rocm-smi --showpids`), `VmSwap 0`; the budget correction moved Hot `675 → 809` and VRAM held `21.99 → 23.76 GiB` at context `256`.
- **Correctness / service**: output `The capital of France is **Paris**.`, stop `eos`, `9` tokens; `--` for per-tier counts, the supply is not instrumented on this path
- **Conclusion**: decode is `≈3 tok/s` in every configuration, and Hot/Cold-only to Hot/Warm/Cold moves it by less than the spread across runs, so no throughput claim is made. Prefill is not amortized (`≈1.13×` decode per prompt token), and Warm is bound by physical RAM (`≈40 GiB`, `45 GiB` swapped and `40 GiB` did not) rather than by the `52.62 GB` budget cap
