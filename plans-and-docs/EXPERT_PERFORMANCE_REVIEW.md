...
- **2026-09-08 — Step 3 measurement (M17), hypothesis rejected**: env-gated instrumentation (`AEON_MEASURE_LOCALITY=1`, per-layer 4-token top-6 history in `V4Pipeline`) measured the coverage of each token's top-6 by the union of the previous n tokens' top-6 sets — the exact ceiling of any history-based speculative prefetcher. Gated layers 3–42 aggregate over 400 pairs: n=1 → 3.38/6 (56%), n=2 → 3.60/6 (60%), n=3 → 3.60/6 (60%), n=4 → 3.44/6 (57%); P(full 6/6) ≤ 27.8%. **The measured VRAM hot-hit rate on the identical run is already 59.3% ≈ the n=2 union ceiling: LRU residency at 664 slots (~2.6 tokens of working set) already harvests essentially all historical routing locality.** A previous-token(s) speculative prefetcher would add ≤1% hits, and the residual ~2.4 misses/layer are genuinely novel experts, unpredictable from routing history. Conclusion: **do not build historical-set speculation**. Latency-bound misses must instead be attacked by (a) making compute fast enough that the exposed read window shrinks relative to it (Step 4), (b) making each miss cheaper (Step 2 contiguous slots + Step 6 contiguous repack), and (c) model-level routing prediction (research track, out of roadmap scope). Caveat: the benchmark's degenerate repetitive output (`223` ×7) inflates locality; a diverse prompt would lower both coverage and hit rate together, leaving the conclusion intact. Step 4 (W4A16 GEMM rewrite, ~108 ms → ~15 ms/token target) is now the highest-priority item. See [Performance Ledger](PERFORMANCE_LEDGER.md) M17.
