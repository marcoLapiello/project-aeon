...
* **Status**: Historical post-M15 review. Steps 1, 2, 4, and 5 were implemented and measured in M16 and M18-M20; same-layer previous-token speculation from Step 3 was measured and rejected in M17. Step 6 and the remaining cold-tier work are still open. Use [EXPERT_PERFORMANCE_REVIEW_CONCLUSIONS.md](../current/EXPERT_PERFORMANCE_REVIEW_CONCLUSIONS.md) and [PERFORMANCE_LEDGER.md](../../status/PERFORMANCE_LEDGER.md) for current conclusions and measurements.
...
The table below is the original post-M15 proposal. Its expected effects are
historical estimates, and later execution results supersede its ordering where
noted in the execution log. In particular, do not treat historical-set
speculation in Step 3 as a current implementation recommendation: M17 found
that the existing LRU already captured essentially all of that locality.
...
**Historical post-Step 1–5 estimate: 25–40 ms/token (25–40 tok/s) at 43 layers**, versus 252 ms at the time — before Phase 3 multi-GPU. This is not a current acceptance target.
...
- **2026-09-08 — Step 1 silicon results (M16)**: all six regression tests passed with identical golden tokens. Controlled A/B: Warm disabled `4.36 tok/s` (unchanged), Warm 35 GiB `5.11 tok/s` (vs M15 `5.15`). **Throughput unchanged despite 3× less traffic per warm hit — conclusive evidence the loop is latency-bound by just-in-time dispatch, not bandwidth-bound.** The original 7–9 tok/s Step 1 estimate was therefore wrong. The result initially prioritized Step 3; M17 then measured same-layer historical-set coverage and rejected that speculative policy. Warm hits dropped 684 → 157 without refill-on-eviction; acceptable, since warm coverage demonstrably does not bind throughput at this stage. See [Performance Ledger](../../status/PERFORMANCE_LEDGER.md) M16.
- **2026-09-08 — Step 4 implemented (M18)**: added `w4a16_gemv_kernel`, a decode-only (M=1) warp-per-row fused INT4 GEMV — coalesced `uint4` weight streams (1 uint4 = one 32-wide scale group), inline FP32 dequant-FMA with dual accumulators, warp-shuffle reduction, no LDS or `__syncthreads`; 2048–4096 warps vs the old kernel's 32–64 blocks. Kernel: `138.8 → 13.9 µs` per expert GEMM (10×, ~340 GB/s effective weight stream), bit-exact vs CPU FP32 reference. End-to-end: warm off `4.36 → 5.25 tok/s`, warm 35 GiB `5.11 → 5.79 tok/s`, TTFT `2755 → 1835 ms`. All regressions pass; golden token `295` preserved; benchmark token sequence flipped to `[237, 201, 1778 ×6]` — deterministic and tier-identical (warm on/off), i.e. FP32 summation-order drift flipping near-tie greedy argmax in the benchmark's repetitive regime, not a numerics bug. See [Performance Ledger](../../status/PERFORMANCE_LEDGER.md) M18.
- **2026-09-08 — Step 2 implemented (M19)**: `UnifiedVRAMExpertPool` re-laid-out to a single contiguous device allocation with per-slot 13.5 MiB regions byte-identical to the `.aeon` layout — full-expert H2D is now **1 `hipMemcpyAsync` instead of 6**; plus a DMA stream split (`sdma_cold_stream` for io_uring staging→VRAM, `sdma_stream` for warm/safetensors H2D) so cold bursts no longer head-of-line block warm hits. A/B: warm 35 GiB `5.79 → 5.88 tok/s`, warm off `5.25 → 5.39 tok/s`, TTFT `1835 → 1803 ms`; tokens identical to M18. All regressions pass; golden token `295` preserved. Modest throughput gain but structurally removes submission overhead and stream coupling; the step remains bound by exposed cold-miss reads. See [Performance Ledger](../../status/PERFORMANCE_LEDGER.md) M19.
- **2026-09-08 — Step 5 implemented (M20)**: eliminated the per-layer router-logits D2H/sync/CPU/H2D round-trip with a device-side half→float kernel (43 pipeline drains/token removed); added a two-phase GPU argmax over the 129,280-logit head (first-max-wins tie-break identical to the CPU scan; replaces the 258 KB D2H + CPU loop with a 4-byte readback); and a vectorized `uint4` FP16 GEMV (8 halves/lane/iter) applied to the router, shared expert, and LM head projections. A/B: warm 35 GiB `5.88 → 7.10 tok/s` (+20.7%), warm off `5.39 → 6.32 tok/s` (+17.3%). All regressions pass; golden token `295` preserved; benchmark tokens returned to the M15 sequence `[237, 223 ×7]`, confirming the M18 flip was tie-break noise. See [Performance Ledger](../../status/PERFORMANCE_LEDGER.md) M20.
