...
| **Fixes** | (a) `reserve_request` fused the VRAM slot into the read, so a read could not be issued without one; (b) **R3's first link was missing**, so the drive was handed whole-layer read waves without regard to what it was already carrying. |
| **Where** | `expert_registry.hpp` (`COLD_STAGED`, `stage_only`, `attach_vram_destination`, `operation_is_staged_only`); `tiered_expert_supply.hpp` (`io_complete`, `enqueue_expert_copy` returns `bool` + late attach, two-phase `materialize_available`, `materialize` joins the two legs); `v4_expert_supply.hpp` (`pump_layer_prefetch` always re-syncs); `v4_prefill_sweep.hpp` (`reads_in_flight`, `advance_reads`, two-term `read_lookahead_capacity`); `v4_model_host.hpp` (ring sized to the arena) |
| **Change** | (a) A cold read reserves **no** VRAM destination; `attach_vram_destination` supplies one when the copy can run, and `enqueue_expert_copy` returns `false` when VRAM is full, leaving the expert staged to retry. `materialize` now **enqueues the copy for every already-`io_complete` expert** before settling reads — without it a pump-deferred copy stayed unsubmitted and the body hit `duplicate request joined before its transfer was submitted`. (b) **One read wave in flight at a time**, the next issued by `advance_reads()` from the per-token hook the instant the previous lands. |
| **Requirement** | R3 ✅ every stage advance is a completion event; the read leg is bounded by staging, the copy leg by VRAM. |
| **Requirement** | R5/R6 ✅ `min(staging blocks, banks − 1)` — the runtime free staging, and nothing else. **No VRAM term** (an earlier attempt added one and it *was* the re-coupling; see analysis §16.4 → §17). |
...
| **Bounding the read queue by free VRAM** | Tried in §16.4 and **refuted in §17**: it *is* the very coupling the decoupling removes (the cartridge box must not be sized by the rifle's magazine wells). The real `3E` defect was two read waves in front of the drive. |
