...
The decisive shape: the swept byte count is **constant at `145.1 GiB`** (one model read) while serial grows at `1.54 GiB/token`, so the advantage rises with the prompt. The overlap is worth `+52%` (N=256) and `+32%` (N=512). The remaining gap is **compute**: ≈`56 s` of `73.6 s` at `N = 512` is the body — `144 ms/token` against colibri's `31 ms/token` (`3324 tokens / 103.9 s`). | `bench_prefill_ab` |
...
| **Load/compute overlap in the sweep** | **built (`2026-09-22`, M42)** — the next layer's reads are dispatched before the current body runs, hiding the drive behind ≈`1.4 s`/layer of compute (`34.6 s → 6.2 s` exposed at `N = 512`). Deepening it past one layer is a further lever only if `io_s` (submission, `11.6 s`) proves to matter |
| **Prefill body compute** | the new bound: `≈144 ms/token` at `N = 512` against colibri's `≈31 ms/token`. Not a supply problem — it is the body at batch width, and it is where the remaining `4.6x` lives |
