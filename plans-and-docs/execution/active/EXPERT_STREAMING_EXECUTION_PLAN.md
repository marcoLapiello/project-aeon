...
4. **Chunk-wide expert dispatch:** `on_routing_ready` / `accumulate_routed` over `C` tokens so the `6C` requests are issued **as a set** (D1); dedup to the layer's distinct set (D2) without permuting slot-sum order; leases released at the **layer boundary** (D3). **[layer-boundary release built; the `6C` set and dedup are the remaining work — the window currently drives the existing per-token dispatch]**
   > **Item 4 and D4 are one change, not two — measured `2026-09-21`.** The routing is produced *inside* the attention tail (after attention), so issuing the `6C` requests as a set requires lifting the router out into a phase of its own: attention for every token, then router for every token, then one dispatch, then MoE for every token (the decomposition colibri's `coli_v4_block_window_batch_ref` uses). Attempting that phase split **alone**, with the per-token dispatch, is *worse than not doing it*: it separates `on_routing_ready` from `on_routed_consumed`, and the staging arena — whose slots are addressed by **layer parity, six per layer** — then hands the same six slots to every token at that layer at once, so the second token's `begin_io` meets a slot the first still holds in `IO_PENDING`. The collision is silent until the pool stops answering Hot: with early tokens fully Hot, no staging is bound and the split appears to pass. **So the phase split, the layer-wide dispatch and the depth-sized free-list arena must land together, or none of them should.** The per-token tail is retained in the chunk until then.
