...
| :--- | ---: | ---: | ---: | ---: |
| chunk-major, `C = 16` | 63 | 3.1 TB | 485 s | 1 MB |
| chunk-major, `C = 256` | 4 | 621 GB | 98 s | 16 MB |
| **layer-major, `W ≥ 1024`** | **1** | **156 GB** | **25 s** | **64 MB** |
| *measured, `W = 1024`, `C = 256`* | *1* | *145 GiB* | *124 s* | *96 MiB* |

**156 GB is the floor** — the whole model read once. `W ≥ N` reaches it. The residual column is the *in-place* carry (D-e below is verified: the body reads only each row's own residual), so it is `min(N, W) × 64 KB`, not doubled.
...
4. **Chunk-wide expert dispatch:** `on_routing_ready` / `accumulate_routed` over `C` tokens so the `6C` requests are issued **as a set** (D1); dedup to the layer's distinct set (D2) without permuting slot-sum order; leases released at the **layer boundary** (D3). **[built — `2026-09-22`, ledger M38]** The chunk's phase 2 is the split the item needs: attention-and-norm for every token, then router for every token (the selections on the host), then **one** `on_routing_ready_batch`, then MoE for every token. `dispatch_layer_prefetch_batch` deduplicates the `6C` requests to the layer's distinct set, stages each once, and returns a per-token index map; the executor's single `accumulate_routed` resolves each token's `k`-th expert through that map, so the fixed-order reduce still sums in `k` order and dedup changes *which copy is read*, never the order. De-duplication measured at `1909` distinct of `4128` draws over the 16-token window, with `window ≡ serial` still byte-exact.
   ...
   **D4, as built.** The arena's slot count is a construction parameter, not the `NUM_BUFFERS × 6` literal. The demand is `min(6C, experts_per_layer)` — dedup can only collapse onto the layer's own experts, so `6C` at a wide chunk overstates it — and the arena is sized to that, floored at decode's `12` and raised to the whole layer when the sweep runs. The direct reader's `io_uring` depth is sized the same way; `dispatch()` queues a whole batch's reads before its single `submit_pending_reads()`, so a queue shallower than `6C × chunks-per-expert` would overflow instead of stream. A layer-wide batch assigns each distinct expert the staging index of its position in the distinct set `0…D-1`, and the graph refuses a chunk that would need more slots than the arena owns rather than colliding two transfers silently. `AeonRuntimeConfig::prefill_chunk` is now a **user setting** (default `64`, capped by the body's row cap of `256`); the arena follows it. The **smaller** concurrency depth (D4's `banks × depth` target) and the waving that reaching it requires remain open, and the measured staging footprint (`3456 MiB` pinned at `C = 256`) is the reason it is worth doing.
...
6. **The layer-ordered sweep (the residency model).** Prefill and decode are two **different allocation strategies**, so the switch between them is a hard switch, and prefill streams in **layer order** rather than evicting by recency. **[built — `2026-09-22`, ledger M40]**
   ...
   **Built (`2026-09-22`, M42) — and it was the whole gap, for one layer of depth.** A layer's reads are now **dispatched before the previous layer's body runs** (`before_layer(L)` issues `L + 1`, then the driver computes `L`), so `before_layer(L + 1)` finds the transfer complete and its wait is near zero. Measured at `N = 512`: blocked-inside-`materialize` `34.6 s → 6.2 s`, and the swept prefill `97.1 s → 73.6 s` (`5.27 → 6.96 tok/s`). One layer of depth is enough because a layer set is ≈`0.55 s` of drive against ≈`1.4 s` of compute — **the plan's own `30 ms` per layer was wrong by ~46x**, which is why the overlap pays here at all. When the pool cannot hold two layers, `dispatch_ahead` is a no-op and the loads fall back to synchronous: the strategy degrades rather than failing.

   **The sweep is engaged by one visible switch, not a threshold.** `AeonRuntimeConfig::prefill_sweep` (default on), feasibility being the only other condition — the Hot pool must hold a whole layer, since the sweep has no victim to evict. An earlier revision gated it on a window-size rule (`6W ≥ 2 × experts_per_layer`) meant to stop a narrow window paying a whole-layer load; the measured cost of that rule was that it silently turned the sweep off on the production path while a gate was moved down until it stopped being slow, which is a test serving itself. Withdrawn (M42), and the narrow-window case is a measurement question, not a hidden switch.
...
7. **The engine's prefill is the window.** `V4Engine::chat` hands the whole prompt to `V4Graph::forward_window` as one unit and leaves the rest to decode. The generation loop is unchanged — `text::generate_token_ids` gained a `PromptPrefill` step (`[prompt] -> first token`) beside its per-token `TokenStep`, so the loop still owns the EOS stop, the cap and the context limit, and the engine supplies a **prefill mechanism** rather than a second loop.
   ...
   **Both knobs are user settings, and the workspace follows them.** `AeonRuntimeConfig::prefill_window` (`--prefill-window`, default `4096` = the reference's segment; `0` = the whole prompt) and `prefill_chunk` (`--prefill-chunk`, default `64`) are read by `V4ModelHost::allocate_prefill_workspace` **at load**, which allocates the residual carry for `W` and the batch scratch for `C` — worst case across all 43 layers — so no window can fail mid-prompt on an allocation and the budget's terms are realised rather than merely reserved. A prompt longer than `W` runs `⌈N/W⌉` windows; a shorter one is a single window at `C = min(C, N)`. **[built — `2026-09-22`, ledger M41, M43]**
...
| **2** | **Tier-invariance preserved** — the M31 comparison still holds on prefill (Hot+Cold vs Hot+Warm → identical logits), proving Warm-frozen changed no number. **Met (`2026-09-22`, M42/M43):** `scripts/expert_tier_invariance.sh` PASSES through the engine's windowed prefill — byte-identical logits, `logical_bytes_from_warm` A `0` / B `46.6 GB`. | `scripts/expert_tier_invariance.sh` |
...
| **4** | **No leak** — `invariants_hold()`, `outstanding_leases() == 0`, `staging_in_use == 0` at end; prefill `demotion_attempts == 0`, `forced_drains == 0`. **Met (`2026-09-22`, M39–M43):** the `[Invariants]` line is unconditional in `aeon_chat` and reads `invariants_hold=true outstanding_leases=0 forced_drains=0 staging_in_use=0` on every run recorded here, including the three engine runs of M43. | `[Invariants]` line |
| **5** | **Throughput** — prefill tok/s against the measured baseline, and bytes/token against §6b's prediction. **Met (`2026-09-22`, M42).** The baseline is now measured in the same harness, one pristine process per arm (`bench_prefill_ab`):

| N | serial | swept (double-buffered) | ratio | swept bytes |
| ---: | ---: | ---: | ---: | ---: |
| 256 | `103.3 s` / `2.48 tok/s` / `394.8 GiB` | **`44.8 s` / `5.71 tok/s`** / `145.1 GiB` | **`2.30x`** | constant |
| 512 | `211.2 s` / `2.42 tok/s` / `786.5 GiB` | **`73.6 s` / `6.96 tok/s`** / `145.1 GiB` | **`2.88x`** | constant |

The decisive shape: the swept byte count is **constant at `145.1 GiB`** (one model read) while serial grows at `1.54 GiB/token`, so the advantage rises with the prompt. The overlap is worth `+52%` (N=256) and `+32%` (N=512).

  **Superseded in part (`2026-09-22`, M43).** The claim that the remaining gap is body compute came from subtracting `load_ns + io_ns` from wall clock, which is unsound — both are host-side waits on a stream that overlaps compute. With the registry's per-request audit removed (M43), the engine's own prefill moved `52.4 → 39.4 s` at `W=512/C=128`, and **`io_ms` — the dispatch preparation — fell `10 162 → 450 ms`**. So a large part of what M42 called "body compute" was bookkeeping. What is left is ~`120 ms/prompt-token` and linear in the prompt; that is the open target, and it is an investigation, not a Step 6 gate. | `bench_prefill_ab`, `aeon_chat` |
...
**Requirement.** Neither knob carries a target in advance: `C` (body chunk, bounds the scratch) and `W` (layer-major window, bounds the residual) — Step 6 §6b. Both are exposed as user settings so the engine is configurable for other hardware rather than tuned for this GPU; what remains is to **sweep** them and measure.
...
**Consequence for the budget.** `PIPELINE_SCRATCH_BYTES = 100 MiB` was a literal wrong in both directions: it over-counted decode scratch by ~an order of magnitude **and did not cover the batch scratch at all**. It is replaced by **three derived terms** — a decode allowance, a batch term from the configured `C`, and a residual-carry term from the configured `W` — and the staging line reads `max(12, min(6C, experts_per_layer))` slots instead of a literal `12`. The two allowances are checked against the real allocations at load, so they are verified rather than trusted.

**Warm and staging are independent budgets (Step 0 D5).** The budget used to *subtract* the staging figure from `warm_host_bytes`, treating the request as one host pool with a transit deduction. That is wrong: staging is a corridor sized by `C`, Warm is expert residency sized by the request, and neither is derived from the other. The requested Warm budget is now honoured in full, and what is checked is the **total** — Warm + staging + reserve must fit the host, or the plan is rejected by name.

**Gate.** The sweep's throughput vs `W` and vs `C`, recorded in the ledger; the budget lines track the configured `W` and `C` with no literal. **Met for the budget and the chunk half** (M43–M44); the window half is deferred by a decision below.

**Status: done (`2026-09-22`, M43, M44).**

*Settings and budget, all four closed.*
1. **Three derived VRAM terms.** `Compute Scratch` now reads `(decode allowance + batch allowance-from-C + residual carry-from-W)`, each reported separately. The decode and batch figures are **allowances checked against the real allocations at load** (`V4ModelHost` fails by name if either exceeds); the carry is exact, derived from `W`.
2. **The staging line is derived.** `max(12, min(6C, experts_per_layer))` slots — matching what the host allocates and what the `[Prefill workspace]` line reports. The literal `12` understated a `C = 256` arena by ~21×.
3. **Warm and staging are two budgets, not one** (D5). The request is no longer reduced by the staging figure; the **total** is checked against the host ceiling.
4. **The chunk half of the sweep is measured flat** (`128 → 256` bought nothing at `117 → 123 ms/prompt-token` while costing `+87 MiB` scratch), confirming the expected shape: the window is the lever.

*Still open.*
1. **The window half of the sweep.** Every measurement to date is one pass (`W ≥ N`), so the `⌈N/W⌉` regime has never executed and the cost of a second pass is unmeasured.
2. **The staging arena's `banks × depth` target.** It is sized to the dedup ceiling because a layer-wide dispatch assigns every distinct expert at once; the smaller waved ring is unbuilt, so the pinned footprint stays at `3.38 GiB` at `C = 256`.
...
| "Size the staging arena from `C`" | False. Staging is a **pipeline buffer** for transfers in transit, sized from transfer-concurrency depth (`banks × depth`). The chunk's deduplicated distinct set is its **ceiling**, not its size: `2 × 6 × C` at `C = 256` is 40.5 GiB pinned, which cannot fit. Dedup is a precondition of the ceiling being finite. **Built (`2026-09-22`):** the arena is `max(12, min(6C, experts_per_layer))` — decode's shape at `C = 1`, the *real* deduped ceiling above it — and the `io_uring` depth matches; reaching the smaller target depth is still open, and the measured `3456 MiB` pinned at `C = 256` is why it is worth doing. |
...
| "The sweep is engaged per window" | **Reversed (`2026-09-22`, M42).** That threshold was withdrawn: it silently disabled the sweep on the production path (a `103`-token prompt swept, an `11`-token one did not) and existed mainly to keep a gate fast, which is a test serving itself. `AeonRuntimeConfig::prefill_sweep` is the only switch and it is visible. |
| "≈`30 ms` of compute per layer" | Wrong by ~`46x`. Measured body time is ≈`1.4 s` per layer at `N = 512` (`≈56 s` of body over `43` layers). This is why one layer of lookahead is enough to hide the drive, and it is the reason the overlap paid: see §6c item 6 and M42. |
| `kMaxTokens = 16` is the chunk ceiling | **Not a bottleneck (`2026-09-22`, M42).** Raised to `64` (colibri's equivalent is `128`) and measured: `5.27 → 5.29 tok/s` at `N = 512`, no change. The chunk is a launch-count knob and launch count is not what bounds this prefill. |
...
| :--- | :--- |
| **Layer-major window `W` and body chunk `C`** | the Step 7 sweep completes. The order is decided (Step 6 D-a); both are settings (M43); the **chunk half is measured flat** (`128 → 256` changed nothing) and the **window half is unmeasured** — every run to date is one pass |
| **In-place residual write-back** (§6.3, Step 6 D-e) | **verified** — the body reads only each row's own residual, and M37 asserts the carry round-trips byte-exactly, so it is `min(N,W) × 64 KB` |
| **Load/compute overlap in the sweep** | **built (`2026-09-22`, M42–M43)** — the next layer's reads are dispatched before the current body runs. At `W = 1024 / C = 256` the blocked load is `4.8 s` of a `124 s` prefill, so the drive is no longer the bound and deeper lookahead has little left to win |
| **The prefill body's `~120 ms/prompt-token`** | **the open target.** Linear in the prompt (`117 ms/tok` at 338, `123` at 1004), measured through `aeon_chat`. Not supply (load `4.8 s`, submission `0.43 s`) and not the registry audit any more (M43). Colibri's `≈31 ms/token` is the reference; this is the next investigation |
| **The staging arena's `banks × depth` target** | open — it is at the dedup ceiling because a layer-wide dispatch assigns every distinct expert at once, costing `3456 MiB` pinned at `C = 256`. The waved ring is unbuilt |
