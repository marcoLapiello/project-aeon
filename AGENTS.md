...
- [x] Stage 1 parallel expert-kernel path: version-2 W4A16 swizzled artifact, vectorized Wave32 GEMV, fused six-expert W1/W3 plus clamped SwiGLU, fused W2 FP32 accumulation, opt-in pipeline/CLI integration, and silicon validation. The version-1 path remains the default; the standalone [Stage 1 implementation report](plans-and-docs/analysis/current/EXPERT_KERNELS_REVIEW_stage-1_IMPLEMENTATION_REPORT.md) records the exact proposal differences. The runtime resolves six complete expert payloads into VRAM and waits for transfer events before launch; the kernels do not handle non-resident experts or storage/cache misses. The expert's activation-staging variant was benchmarked and was not adopted: it was slower for W1/W3 and neutral for W2.
