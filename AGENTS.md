...
- [x] **Tier 1 oracle harness + two certified primitives.** `reference/dsv4_oracle.hpp` is the host-only, fp64, kernel-free reference layer (RMSNorm + the two-class RoPE spec). `kernels/v4_norm.hpp` and `kernels/v4_rope.hpp` are the first two kept primitives extracted so each can be gated on its own; `v4_attention.hpp` includes both. Each gate runs the oracle's own self-checks first, states an explicit tolerance, and asserts discriminating properties rather than only closeness — RMSNorm passes at one fp16 ulp; RoPE passes 18 lines covering both bases, the tail-only slice (trap 27), and forward∘inverse.
- [ ] **Next: Tier 1 primitives.** Certify in dependency order against `dsv4_oracle.hpp`: MLA q/kv paths (incl. both intermediate norms), HC project + Sinkhorn, attention score + sink + softmax, compressor (+ APE), indexer + top-k, grouped output, router, expert matmul, shared expert. One op per gate.
