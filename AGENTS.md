...
- [ ] **[Phase 2 Execution Plan](plans-and-docs/PHASE_2_EXECUTION_PLAN.md) — Single-GPU 3-Tier Storage & Memory Hierarchy Optimization**:
  ...
  - [ ] Spike 3: Linux `io_uring` Direct I/O NVMe Cold Tier integration.
    ...
    - [ ] **Next (highest priority)**: Review Step 5 — remove per-layer CPU stalls (single-sync router path, GPU argmax over the 129,280-logit head to kill the 258 KB D2H + CPU scan, vectorized FP16 GEMVs); then Step 6 (contiguous `.aeon` repack toward the ≥6 GB/s cold-tier target). The step is dominated by the exposed just-in-time cold-miss read path (~170 ms/token, 1,077 cold misses/run).
