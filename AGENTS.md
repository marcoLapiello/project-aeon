...
- [ ] **Next: Tier 2 — the layer body.** Compose the certified primitives into one layer and gate the composition rather than the parts; then compare formatted inputs, intermediate checkpoints, and final logits against a trusted compatible reference. The layer body must be the same code as decode, parameterised by chunk size. **Apply mutation testing to each Tier-2 gate as it is written** — a gate prints PASS until a wrong implementation has been shown to fail it.
