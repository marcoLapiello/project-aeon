...
- [x] **Obsolete tests removed.** The pre-rewrite layer-schedule tests and the circular kernel checks were deleted; the two independent-oracle parity tests stay gated as tier-0 anchors. Default `ctest`: 32 tests; legacy: 34.
...
- [ ] **Next: Tier 2 item 18 — serial multi-token decode across compressor boundaries.** Both Tier-2 gates re-seed the residual from the oracle between steps, so they measure one layer's composition, not the drift or the state evolution of a long loop. Item 18 is where the residual must stop being re-seeded and the classes must run one after another in a real serial decode with genuine state carried forward.
- [ ] **Tier 2 remains:** item 18 (serial multi-token decode). The layer body is still deliberately **not** wired into `core/v4_pipeline.hpp`: that file is the pre-rewrite graph, gated off, and the rewrite must not depend on it. The body takes its RoPE tables, an attention-trace observer, and a routed-expert executor as parameters, so the same code serves decode, batched prefill and the gates.
