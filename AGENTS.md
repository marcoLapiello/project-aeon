...
- [x] **Tier 1 oracle harness + six certified primitives.** `reference/dsv4_oracle.hpp` is the host-only, fp64, kernel-free reference layer (RMSNorm, the two-class RoPE spec, the dense-projection and MLA references, Hyper-Connections, attention score+sink+softmax, and the compressor + APE). `kernels/v4_norm.hpp`, `kernels/v4_rope.hpp`, and `kernels/v4_gemv.hpp` are the kept primitives extracted so each can be gated on its own; `v4_attention.hpp` includes all three. Each gate runs the oracle's own self-checks first, states an explicit tolerance, and asserts discriminating properties rather than only closeness — RMSNorm at one fp16 ulp; RoPE 18 lines covering both bases, the tail-only slice (trap 27) and forward∘inverse; MLA 9 lines covering the low-rank Q path with both norms at full dimensions; HC 14 lines covering the comb index convention (trap 34), the third HC scale, the 20-iteration count and the asymmetric eps placement; attention 19 lines covering the exact window boundary and the sink as a pure scalar rescale (trap 35); compressor 32 lines covering APE as a score-only term, both ratio classes, the overlap segments and the truncated first entry.
- [ ] **Next: Tier 1 primitives.** Certify in dependency order against `dsv4_oracle.hpp`: indexer + top-k, grouped output, router, expert matmul, shared expert. One op per gate.
