...
### Current priority
- [ ] **Model correctness:** continue [the model correctness plan](plans-and-docs/execution/active/MODEL_CORRECTNESS_EXECUTION_PLAN.md) at Stage 1: independently validate real dense projections and routed expert outputs, then recover the checkpoint-driven SWA/CSA/HCA/indexer graph, compressed prefill state, configured RoPE/YaRN behavior, and trusted-reference parity before using routing data for placement decisions.
