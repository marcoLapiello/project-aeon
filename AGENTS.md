...
- [x] Single model-derived V4 runtime path: `V4Pipeline::initialize()` owns the production startup contract, derives model dimensions from package metadata, and retired obsolete partial-layer pipeline tests and initialization modes.
- [x] V4 pipeline ownership reduction: model-level allocations moved to `V4ModelResources`, and the architecture-neutral Hot/Warm/Cold transfer, prefetch, direct-I/O materialization, demotion, readiness, and supply telemetry state moved to `TieredExpertSupply`; `V4ExpertSupplyCoordinator` now retains only the six-expert request adapter and Aeon source mapping without changing full-model behavior.
