...
- [x] **Tier 1 oracle harness + four certified primitives.** `reference/dsv4_oracle.hpp` is the host-only, fp64, kernel-free reference layer (RMSNorm, the two-class RoPE spec, the dense-projection and MLA references, and Hyper-Connections). `kernels/v4_norm.hpp`, `kernels/v4_rope.hpp`, and `kernels/v4_gemv.hpp` are the kept primitives extracted so each can be gated on its own; `v4_attention.hpp` includes all three. `hc_sinkhorn.hpp` was already separate. Each gate runs the oracle's own self-checks first, states an explicit tolerance, and asserts discriminating properties rather than only closeness — RMSNorm at one fp16 ulp; RoPE 18 lines covering both bases, the tail-only slice (trap 27) and forward∘inverse; MLA 9 lines covering the low-rank Q path with both norms at full dimensions; HC 14 lines covering the comb index convention (trap 34), the third HC scale, the 20-iteration count and the asymmetric eps placement.
- [ ] **Next: Tier 1 primitives.** Certify in dependency order against `dsv4_oracle.hpp`: attention score + sink + softmax, compressor (+ APE), indexer + top-k, grouped output, router, expert matmul, shared expert. One op per gate.
