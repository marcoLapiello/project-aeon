...
- [ ] **[Phase 2 Execution Plan](plans-and-docs/PHASE_2_EXECUTION_PLAN.md) — Single-GPU 3-Tier Storage & Memory Hierarchy Optimization**:
  ...
  - [ ] Spike 3: Linux `io_uring` Direct I/O NVMe Cold Tier integration.
    ...
    - [ ] **Next (highest priority)**: Review Step 2 — contiguous per-slot VRAM layout (1 memcpy/expert instead of 6) + DMA stream split; then Step 5 (single-sync router, GPU argmax, vectorized FP16 GEMVs) and Step 6 (contiguous `.aeon` repack). With GEMM time down to ~11 ms/token, the exposed just-in-time miss path now dominates the ~173 ms step.
