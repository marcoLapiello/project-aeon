...
- [x] **Tier 1 oracle harness + first certified primitive.** `reference/dsv4_oracle.hpp` is the host-only, fp64, kernel-free reference layer; `kernels/v4_norm.hpp` is the first kept primitive extracted so it can be gated on its own; `tests/test_v4_norm_oracle.cpp` is the gate pattern (independent reference, explicit tolerance, oracle self-check). RMSNorm passes at one fp16 ulp.
- [ ] **Next: Tier 1 primitives.** Certify in dependency order against `dsv4_oracle.hpp`: RoPE (fwd + inv, two bases), MLA q/kv paths, HC project + Sinkhorn, attention score + sink + softmax, compressor, indexer + top-k, grouped output, router, expert matmul, shared expert. One op per gate.
